...
.table-container {
	...
	@method('PUT')
			...
			<tr>
				...
				<textarea readonly class="form-field" name="shipper" style="width:98%; height: 80%;">{{ old('shipper',$data->shipper) }}</textarea>
				</td>
			...
			<tr>
				...
				<textarea readonly class="form-field" id="consignee" name="consignee" style="width:98%; height: 80%;">{{ old('consignee',$data->consignee) }}</textarea>
			</td>
			...
			<tr>
				...
				<textarea readonly class="form-field" id="notify_party" name="notify_party" style="width:98%; height: 80%;">{{ old('notify_party',$data->notify_party) }}</textarea>
			</td>
				...
				<td class="no-border-rb" colspan="3">
					...
					<table style="width: 300px; margin: 0;">
						...
						<input readonly id="date_of_issue" value="{{ old('date_of_issue',$data->date_of_issue) }}" type="date" name="date_of_issue" style="width:87%;"/>
					</td>
						<td class="no-border">B/L NUMBER<br>
						<input readonly id="bl_number"  value="{{ old('bl_number',$data->bl_number) }}" type="text" name="bl_number" style="width:55%;"/>
					</td>
						</tr>
					
						<tr>
						<td class="no-border-rbl" colspan="3" rowspan="2">FOR DELIVERY OF GOODS PLEASE APPLY TO:<br>
						<textarea readonly class="form-field" id="for_delivery_apply_to" name="for_delivery_apply_to" style="width:98%; height: 100%;">{{ old('for_delivery_apply_to',$data->for_delivery_apply_to) }}</textarea>
					</td>
						</tr>
			...
			<tr>
				...
				<input readonly id="place_of_receipt" value="{{ old('place_of_receipt',$data->place_of_receipt) }}" type="text" name="place_of_receipt" style="width:90%;"/>
			</td>
				<td class="no-border-left">PORT LOADING<br>
				<input readonly id="port_of_loading" value="{{ old('port_of_loading',$data->port_of_loading) }}" type="text" name="port_of_loading" style="width:90%;"/>
			</td>
            ...
            <tr>
				...
				<input readonly id="ocean_vessel" value="{{ old('ocean_vessel',$data->ocean_vessel) }}" type="text" name="ocean_vessel" style="width:90%;"/>
			</td>
				<td class="no-border-left">VOYAGE NO.<br>
				<input readonly id="voyage_no" value="{{ old('voyage_no',$data->voyage_no) }}" type="text" name="voyage_no" style="width:90%;"/>
			</td>
	
			</tr>
            <tr>
				<td class="no-border" style="height: 35px;">PORT OF DISCHARGE<br>
				<input readonly id="port_of_discharge" value="{{ old('port_of_discharge',$data->port_of_discharge) }}" type="text" name="port_of_discharge" style="width:90%;"/>
			</td>
				<td class="no-border-left">FINAL DESTINATION<br>
				<input readonly id="final_destination" value="{{ old('final_destination',$data->final_destination) }}" type="text" name="final_destination" style="width:90%;"/>
			</td>
				<td class="no-border-right">FREIGHT PAYABLE AT<br>
				<input readonly id="freight_payable_at" value="{{ old('freight_payable_at',$data->freight_payable_at) }}" type="text" name="freight_payable_at" style="width:50%;"/>
			</td>
				<td class="no-border" colspan="2">NUMBER OF ORIGINAL B/L<br>
				<input readonly id="number_of_original_bl" value="{{ old('number_of_original_bl',$data->number_of_original_bl) }}" type="text" name="number_of_original_bl" style="width:90%;"/>
			</td>
			</tr>
			</table>
